...
def GetTime():
    ...
    # Then we split our time by the colins to get the numbers only,
    # and store them to seperate variables
    h, m, s = data.split(':')
    
    # As this is a 12 hour clock, we need to make our hour be properly changed
    # to be 12 hour time instead of 24 hour time with the mod operator (%),
    # which returns the remainder of division.
    h %= 12
    
    # Finaly, we just have to put the data back together, using the join
    # function to stick colins back in.
    data = ':'.join([h, m, s])
